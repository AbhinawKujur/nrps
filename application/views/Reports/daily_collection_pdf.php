...
<body>
	<div>
		...
		<center>
			...
			<p style='font-size:20px; position:relative; top:-10px;'><?php echo $School_Address; ?></p>
			<p style='font-size:20px; position:relative; top:-15px;'>Session (<?php echo $School_Session; ?>)</p>
		</center>
	</div>
	<table width='100%'>
		<tr>
			<th style='font-size:16px!important;'>Collection Type:- <?php echo $collection_type; ?></th>
			<th width='50%' style='font-size:16px!important;'>
				<center>Daily Collection Report as on <?php echo date('d-M-Y', strtotime($date_type)); ?></center>
			</th>
			<th style='font-size:16px!important;'>Collection Counter:- <?php echo ($counter != 'All Counters')?(($counter  == 'EMP0104')?'School':'Bank'):$counter; ?></th>
		</tr>
	</table>
	<hr>
	<table border='1'>
		...
		<tbody>
			...
			?>
					<tr>
						...
						<td><?php echo $data_type->Payment_Mode . '-' . $data_type->CHQ_NO; ?></td>
					</tr>
			<?php
					$i++;
					$t = $t + $data_type->TOTAL;
					...
					if ($data_type->Payment_Mode == 'CASH') {
						$cash = $cash + $data_type->PAID_AMOUNT;
						//$cash = $tpaid;
					} elseif ($data_type->Payment_Mode == 'CARD SWAP') {
						$card = $card + $data_type->PAID_AMOUNT;
					} elseif ($data_type->Payment_Mode == 'CHEQUE') {
						// $cheque = $cheque+$data_type->TOTAL;
						$cheque = $cheque + $data_type->PAID_AMOUNT;
					} elseif ($data_type->Payment_Mode == 'ONLINE' || $data_type->Payment_Mode == 'Online') {
						// $online = $online+$data_type->TOTAL;
						$online = $online + $data_type->PAID_AMOUNT;
					} elseif ($data_type->Payment_Mode == 'BQR') {
						// $bqr = $bqr+$data_type->TOTAL;
						$bqr = $bqr + $data_type->PAID_AMOUNT;
					} elseif ($data_type->Payment_Mode == 'NEFT') {
						// $neft = $neft+$data_type->TOTAL;
						$neft = $neft + $data_type->PAID_AMOUNT;
					} else {
					}
	...
	<div style="height:300px; width:800px; position:relative; top:-10px;">
		<table>
			...
			<tr>
				<td style='font-size:18px!important;'>Recovered Short Amount</td>
				<td style='font-size:18px!important;'>:&nbsp;<?php echo $rsa; ?></td>
			</tr>
			<tr>
