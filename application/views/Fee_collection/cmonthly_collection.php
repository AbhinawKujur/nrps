...
function Cancel_Payment_Counter(){
	...
	if(adm != ""){
		$.ajax({
			url:"<?php echo base_url('Cmonthly_collection/cancel_payment'); ?>",
			type:"POST",
			data:{adm:adm},
...
function adm_function()
{
	...
	if(val!="")
	{
		...
		$.ajax({
		url:"<?php echo base_url('Cmonthly_collection/monthly_adm_data'); ?>",
		type:"POST",
		data:{val:val},
		beforeSend:function(){
			$('.loader').show();
					...
					if(user[1] > 0){
						...
						var r = confirm("This Student Has Previous Year Dues of Rs:-"+ user[1]+" If You Want To collect Then Press Ok Or You Can Cancel.");
						  if (r == true) {
							...
							$.ajax({
							url:"<?php echo base_url('Cmonthly_collection/stu_data'); ?>",
							type:"POST",
							data:{val:val},
...
function show_student_details()
{
	...
	$("#spnr_dsp").show(1000);
	$.ajax({
		url:"<?php echo base_url('Cmonthly_collection/show_student'); ?>",
		type:"POST",
		success:function(data)
		{
			var user = JSON.parse(data);
