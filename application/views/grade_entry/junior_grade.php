...
<script>
  function classes(val){
	  $.post("<?php echo base_url('cosoco_grd/JuniorGrd/classess'); ?>",{val:val},function(data){
  ...
  function exam_type(){
	...
	var sec    = $("#sec").val();
	$.ajax({
		url: "<?php echo base_url('cosoco_grd/JuniorGrd/getSubject'); ?>",
		type: "POST",
  ...
  function sorybyview(val){
	...
	//   alert(exm_code);
	  $.post("<?php echo base_url('cosoco_grd/JuniorGrd/stu_list'); ?>",{sortval:sortval,sub:sub,Class_No:Class_No,sec:sec,exm_code:exm_code,trm:trm},function(data){
		  $("#loading2").hide();
		  $("#stu_list").html(data);
	  });
  }
  ...
  $("#form").on("submit", function (event) {	
	...
	var exm_code = $("#exm_typ").val();
		...
		$.ajax({
			url: "<?php echo base_url('cosoco_grd/JuniorGrd/save_upd_validate'); ?>",
			type: "POST",
			data: $("#form").serialize()+ "&classs=" + classs + "&sec=" + sec  + "&sub=" + sub + "&trm=" + trm + "&exm_code=" + exm_code,
			success: function(data){
				$("#process").hide();
				$.toast({
					heading: 'Success',
