...
<div class="content-wrapper">
						...
						<fieldset class="scheduler-border">
							...
							<div class='col-sm-12'>
								<div class="form-group">
									...
									<input type="text" value="<?php echo $stuData[0]['distance']; ?>" class="form-control" name='distance' id='distance' onkeypress='return event.charCode >= 48 && event.charCode <= 57 || event.charCode == 46' required>
								</div>
							</div>	
							</div>	
  ...
  <script type="text/javascript">
	...
	$(document).ready(function(){
		...
		if(val == 'YES'){
			$("#grand_parent").html("<?php foreach($grand_parent as $key => $val){ if($key != 4){  ?><option value='<?php echo $key; ?>' <?php if($key == $stuData[0]['grand_parent']){ echo 'selected'; }?>><?php echo $val; ?></option><?php } } ?>");
		}else{
			$("#grand_parent").html("<?php foreach($grand_parent as $key => $val){ if($key == 4){  ?><option value='<?php echo $key; ?>'><?php echo $val; ?></option><?php } } ?>");
		}
	});
    ...
    $.ajax({
			url: "<?php echo base_url('adm_nur/Stu_list/updForm'); ?>",
			type: "POST",
			data:formData,
            cache:false,
            contentType: false,
			...
			success: function(data){
				...
				setTimeout(function(){
					window.location="<?php echo base_url('adm_nur/Stu_list/index'); ?>";
				},1500);
			}
		});
	 });
	 ...
	 function reason_save(){
		 ...
		 $.ajax({
			url: "<?php echo base_url('adm_nur/Stu_list/rejectReasonSave'); ?>",
			type: "POST",
