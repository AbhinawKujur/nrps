									...
									<div class='table-responsive'>
										<table class='table' id='example' style="font-size: 12px;">
											...
											<tbody>
												...
												foreach ($issuedbok as $key => $val) {
												?>
													<tr id="a<?php echo $id; ?>" onclick="acce_no('<?php echo $val['BookID']; ?>','a<?php echo $id; ?>')" class='al' style="cursor:pointer">
														...
														<?php if(!empty($val['stu_nm1'])){ echo $val['stu_nm1'];}
																 elseif(!empty($val['stu_nm2'])) { echo $val['stu_nm2'];} 
																 elseif(!empty($val['stu_nm3'])) { echo $val['stu_nm3'];}
																 elseif(!empty($val['stu_nm4'])) { echo $val['stu_nm4'];}
																 elseif(!empty($val['stu_nm5'])) { echo $val['stu_nm5'];}
																 else{echo '';}
																 ?>
																</td>
													</tr>
...
<script>
	...
	function reisu() {
		...
		if (reisucount == 2) {
			...
			if (cnf == true) {
				if (duedate != "" && ids != "") {
					$.post("<?php echo base_url('library/StudentBookReturn/bookreisu'); ?>", {
						duedate: duedate,
						ids: ids
					}, function(data) {
	...
	function returned() {
		...
		if (cnf == true) {
			...
			if (duedate != "" && ids != "") {
				$.post("<?php echo base_url('library/StudentBookReturn/bookreturn'); ?>", {
					'duedate': duedate,
					'ids': ids,
					'rdate': rdate,
					'accno': accno,
	...
	function acce_no(acce_no, accno_id) {

		$('#accnn').val(acce_no);
		var accno = $('#accnn').val();

		$('.al').css("background-color", 'white');
		$('#' + accno_id).css("background-color", '#00e6ac');

		var accno = $('#accno').val();

		var rdate = $('#rdate').val();

		$.post("<?php echo base_url('library/StudentBookReturn/bookDetails'); ?>", {
				acce_no: acce_no
			},
