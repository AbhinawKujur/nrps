...
<section class="content-header">
  <div class="container-fluid">
	<div class="row mb-2">
	  <div class="col-sm-6">
		<h1>Not Attempted List</h1>
	  </div>
	  <div class="col-sm-6">
		<ol class="breadcrumb float-sm-right">
		  <li class="breadcrumb-item"><a href="#">Home</a></li>
		  <li class="breadcrumb-item active">Not Attempted List</li>
		</ol>
	  </div>
	</div>
  </div>
	  ...
	  <div class="card">
		<div class="card-body">
		  <table id="example1" class="table table-bordered table-striped">
			...
			<tbody id='load'>
			<?php
				foreach($notAttemptedReportData as $key => $val){
					?>
						<tr>
...
<script>
	...
	$("#notAttempted_menu").addClass('active');
	
	function startDate(value){
		$("#end_date").prop('disabled',false);
		$.ajax({
			url: "<?php echo base_url('adm_three/reports/NotAttempted/ReportStuData'); ?>",
			type: "POST",
			data: {value:value},
	...
	function endDate(){
		...
		$.ajax({
			url: "<?php echo base_url('adm_three/reports/NotAttempted/ReportStuDateRange'); ?>",
			type: "POST",
