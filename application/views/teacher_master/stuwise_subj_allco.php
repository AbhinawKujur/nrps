		...
		<div class="col-sm-4">
			<table class="table">
				...
				<tr>
					<th>Subject6</th>
					<td>
						<select name="subject6" id="subject6" class="form-control" onchange="subject()">
							<option value="">Select</option>
						</select>
					</td>
				</tr>
...
<script>
	function clswisallo(val) {
		...
		$.post("<?php echo base_url('Teacher_master/classsec'); ?>", {
			classs: classs
		}, function(data) {
			$("#sec").html(data);
		});
	...
	function sec() {
		...
		$.post("<?php echo base_url('Teacher_master/section'); ?>", {
			classs: classs
		}, function(data) {
			$("#subject").html(data);
			$("#subject6").html(data);
	...
	function subject() {
		...
		var subj_code6 = $("#subject6 option:selected").val();
		$.post("<?php echo base_url('Teacher_master/allco_stu_list'); ?>", {
			classs: classs,
			sec: sec,
			subj_code: subj_code,
			subj_code6: subj_code6
		}, function(data) {
			$("#stu_view").html(data);
			$("#loder").hide();
		});
