...
class Verified_list extends MY_Controller {
	...
	public function index(){
		...
		$data['verifiedData'] = $this->alam->selectA('nursery_adm_data','*,(select CLASS_NM from classes where Class_No=nursery_adm_data.class_0)classnm_0,(select SECTION_NAME from sections where section_no=nursery_adm_data.sec_0)secnm_0,(select CAT_DESC from category where CAT_CODE=nursery_adm_data.category)catnm,(select Rname from religion where RNo=nursery_adm_data.religion)religionnm',"f_code='Ok' AND verified_status='1'");
		$this->nurseryAdmissionAdminTemplate('nur_adm/admin/verifiedList',$data);
	}
}
