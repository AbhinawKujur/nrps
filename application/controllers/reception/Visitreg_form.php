...
class Visitreg_form extends MY_Controller
{
	...
	public function index()
	{
		...
		$data['dept'] = $this->db->query("SELECT * FROM `department`")->result();
	...
	public function add_visit()
	{
		...
		$stuid_data = $this->farheen->select('student', '*,(select CLASS_NM from classes where Class_No=ADM_CLASS)stu_class,(select SECTION_NAME from sections where section_no=ADM_SEC)stu_sec', "Student_Status='ACTIVE'");
	...
	public function save_visit()
	{
		...
		if (($this->input->post('mode_id')) != 3) {
			...
			redirect('reception/Visitreg_form/index');
		}
	}
	...
	public function update_visit()
	{
		...
		// $id = $this->input->post('reg_form');
		// if (!$id) {
		// 	echo 'ID is required!';
		// 	die;
		// }

		$vis_date = date('Y-m-d', strtotime($this->input->get('vis_date')));
		...
		$array = array(
			...
			'supp_id' => $this->input->post('sup_id'),
			'entry_mode' => $this->input->post('mode_id'),
			'visit_date' => $this->input->post('vis_date'),
			'name' => $this->input->post('name'),
			'mobile' => $this->input->post('mobile'),
			'otp' => $this->input->post('otp'),
			'dept_id' => $this->input->post('dept'),
		...
		$this->farheen->update('visitreg_form', $array, "id = $id");
		 $last_id =$this->db->insert_id();
	...
	public function stu_fil()
	{
		...
		$stu_details = $this->db->query("select *,(select CLASS_NM from classes where Class_No=ADM_CLASS)stu_class,(select SECTION_NAME from sections where section_no=ADM_SEC)stu_sec from student where STUDENTID='$stu_id'  and Student_Status='ACTIVE'")->result();
...
?>
	...
	function depp_vis()
	{
		...
		$seledept = $this->input->post('depp_viss');

		$strvis_det = $this->db->query("select *,(select mode_type from mode_type where id=entry_mode)entrymode,(select dept from department where id=dept_id)dept,(select vis_purpose from visitor_purpose where id=vis_pur_id)purpose,(select vis_type from visitor_type where id=vis_type_id)type from visitreg_form where dept_id='$seledept'")->result();
		...
		$i = 1;
	?>
		<table class='table table-bordered table-responsive' id='data_table'>
			<thead>
				<tr>
					<th>Sl. no.</th> 
					<th>Actions</th> 
			...
			<tbody>
				...
				?>
					<tr>
						<td><?php echo $i; ?></td>
						<td>
							<a href="<?php echo base_url(); ?><?php echo 'reception/Visitreg_form/edit_form/' . $value->id; ?>" target="_blank" style='color:white;background-color:green;padding:5px'><i class="fa fa-edit" aria-hidden="true" style='color:white'></i> Edit</a>
							<a href="<?php echo base_url(); ?><?php echo 'reception/Visitreg_form/download_form/' . $value->id; ?>" target="_blank" style='color:white;background-color:red;padding:5px'><i class="fa fa-print" aria-hidden="true" style='color:white'></i> Print</a>
...
<?php
	...
	public function edit_form($id)
	{
		...
		$data = $this->db->query("SELECT * FROM visitreg_form WHERE id = $id")->row();
		$mode_data = $this->farheen->select('mode_type', '*', "status='Y'");
		$dept_data = $this->farheen->select('department', '*', "status='Y'");
		$pur_data = $this->farheen->select('visitor_purpose', '*', "status='Y'");
		$type_data = $this->farheen->select('visitor_type', '*', "status='Y'");
		...
		//echo '<pre>';
		//print_r($data);
		$this->render_template('reception/edit_form', $arrayy);
	}
}
