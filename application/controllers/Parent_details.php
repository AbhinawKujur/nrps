...
class Parent_details extends MY_controller {
	...
	public function student_profile(){
		...
		$student_details = $this->dbcon->show_student($adm_no);
	...
	public function stu_attendance(){
		...
		foreach ($monthList as $key => $value) {
			...
			$present_days = $this->sumit->fetchSingleData('COUNT(DISTINCT(att_date))total_present',$table,"att_status IN ('P','HD') AND month(att_date)='".$value['month_code']."' AND admno = '$adm_no' AND year(att_date)='$yearr' ");

			$absent_days = $this->sumit->fetchSingleData('COUNT(DISTINCT(att_date))absent_days',$table,"att_status ='A' AND month(att_date)='".$value['month_code']."' AND admno = '$adm_no' AND year(att_date)='$yearr'");
		...
		//=========================================================//
				...
				//-getting feedetails from the daycoll and tem_daycoll //
					foreach($MON_FEE as $key=>$value){
						$temp_data = $this->dbcon->selectSingleData('temp_daycoll','TOTAL',"RECT_NO='$value'");
						if(empty($temp_data)){
							$daycoll_data = $this->dbcon->selectSingleData('daycoll','TOTAL',"RECT_NO='$value'");
	...
	public function downlad_pdf(){
		$stu_rect = $this->input->get('rectno');
		$adm_no = $this->input->get('admno');
		...
		$this->load->view('parents_dashboard/online_recpt_report_pdf',$report_data);
	...
	public function report_data(){
		...
		//$this->dompdf->setPaper('A4', 'portrait');
		//$this->dompdf->render();
		//$this->dompdf->stream($stu_rect.".pdf", array("Attachment"=>0));
	}
}