...
class Bank_calculation extends MY_Controller
{
	...
	public function get_pay_details()
	{
		...
		if (isset($student_data)) {
			...
			$stu_aprfee   = $student_data[0]->APR_FEE; //N/A
			$adm_status   = $student_data[0]->mid_session_admission_status; // 0
			$Admission_month = $student_data[0]->Admission_month; //1 APR //3 JUN
			$mid_payment_status = $student_data[0]->mid_session_payment_status; //NULL
		}
		...
		foreach ($month_data as $value) {
			...
			for ($i = 1; $i <= 25; $i++) {
				...
				@$fee_head[$i] 	= $feehead[$i][0]->FEE_HEAD; // all 25 head , like tuiton fee, pupil fund,transport fee. . . . . .  . . . . . 
				@$monthly[$i] 	= $feehead[$i][0]->MONTHLY;  // 1 , 0
				@$CL_BASED[$i]	= $feehead[$i][0]->CL_BASED; //1 , 0
				...
				$feeclw_AMOUNT[$i]   = $feeclw[0]->AMOUNT; //4200
				$feeclw_EMP[$i]      = $feeclw[0]->EMP;	  //0
				$feeclw_CCL[$i]      = $feeclw[0]->CCL;	  //0
				$feeclw_SPL[$i]      = $feeclw[0]->SPL;   //0
				$feeclw_EXT[$i]      = $feeclw[0]->EXT;   //4200
				$feeclw_INTERNAL[$i] = $feeclw[0]->INTERNAL; //0
						...
						if ($CL_BASED[$i] == 1) // calculation on the basis of class base //
						{
							...
							//till here
								...
								switch ($emp_ward) {
									case 1:
										$amt_fee = $feeclw_AMOUNT[$i];
										// echo 'case 21 '.$amt_fee.'<br>';
										// echo 'i = '.$i.'<br>';
										break;
									case 2:
										$amt_fee = $feeclw_EMP[$i];
										// echo 'case 22 '.$amt_fee.'<br>';
										// echo 'i = '.$i.'<br>';
										break;
									case 3:
										$amt_fee = $feeclw_CCL[$i];
										// echo 'case 23 '.$amt_fee.'<br>';
										// echo 'i = '.$i.'<br>';
										break;
									case 4:
										$amt_fee = $feeclw_SPL[$i];
										// echo 'case 24 '.$amt_fee.'<br>';
										// echo 'i = '.$i.'<br>';
										break;
									case 5:
										$amt_fee = $feeclw_EXT[$i];
										// echo 'case 25 '.$amt_fee.'<br>';
										// echo 'i = '.$i.'<br>';
										break;
									case 6:
										$amt_fee = $feeclw_INTERNAL[$i];
										// echo 'case 26 '.$amt_fee.'<br>';
										// echo 'i = '.$i.'<br>';
										break;
									default:
										$amt_fee = 0;
		...
		$this->load->view('Bank_collection/bank_feehaead_calculation_monthwise', $array);
	}
