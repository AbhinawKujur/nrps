...
class Online_paymentcal extends MY_Controller{
	...
	public function show_student()
	{
	  $adm_no  	= $this->input->post('adm');
		...
		$getclass = $this->farheen->select('student','DISP_CLASS,DISP_SEC',"ADM_NO='$adm_no' AND Student_Status='ACTIVE'");
		$CLASS_NM=$getclass[0]->DISP_CLASS;
		$SEC_NM=$getclass[0]->DISP_SEC;
		
		if($CLASS_NM=="XI PCM" || $CLASS_NM=="XI COM." || $CLASS_NM=="XII SC."|| $CLASS_NM=="XII COM."|| $CLASS_NM=="XI ARTS"|| $CLASS_NM=="XII ARTS"|| $CLASS_NM=="XI PCB"|| $CLASS_NM=="XII PCM"|| $CLASS_NM=="XII PCB"|| $CLASS_NM=="IX")
		{
		$boardfee = $this->farheen->select('daycoll','sum(fee20)as fee6',"ADM_NO='$adm_no'");
		if($boardfee[0]->fee6=="0")
		{
		$status="Yes";
		...
		//	$mnth_new = strtoupper(date('M'));
		//	$chkbox_new = $this->input->post('chkbox');
		//	$month_new = [];
		//	foreach($chkbox_new as $key => $val){
		//		if($val == 1){
		//			$month_new[] = 'apr';
		//		}else if($val == 2){
		//			$month_new[] = 'may';
		...
		//		}
		//	}
		//	$field = implode(',', $month_new);
		//	$getData = $this->alam->selectA('late_fine',$field,"month='$mnth_new'");
		//	$tot_new = [];
		//	foreach($month_new as $key => $val){
		//		$tot_new[] = $getData[0][$val];
		//	}
		//	$newSum = 0;
		//	foreach($tot_new as $key => $val){
		//		$newSum +=$val;
		//	}
		//	//end late fine calculation//
	...
	$this->session->set_userdata('total_amountt',$total_amount);
			//$this->session->set_userdata('total_amountt',"2.00");
			...
			'amt_feehead6' => $final_amount[6], //$final_amount[6],
	...
	public function payment()
	{
		...
		$exitsData = $this->farheen->select('daycoll','count(*)cnt',"ADM_NO='$adm_no' AND PERIOD LIKE '%$substr1%' and substr(period,1,3)<>'PRE' and substr(period,1,4)<>'MISL'");
		$cnt = $exitsData[0]->cnt;
		
		if($cnt == 0)
		{
		$redirect_url=base_url('Onparent_details/respon');
		$cancel_url=base_url('Onparent_details/pay_details');
		$this->session->set_userdata('merchant_id','260186');
		$this->session->set_userdata('currency','INR');
		$this->session->set_userdata('redirect_url',$redirect_url);
		$this->session->set_userdata('cancel_url',$cancel_url);
		...
		$data['billing_name'] = $this->session->userdata('ffms');
		...
		$ins_data = array(
			...
			'fee8' => ($fee8 < 0)?0:$fee8,
			'fee9' => ($fee9 < 0)?0:$fee9,
			'fee10' => ($fee10 < 0)?0:$fee10,
			'fee11' => ($fee11 < 0)?0:$fee11,
			'fee12' => ($fee12 < 0)?0:$fee12,
			'fee13' => ($fee13 < 0)?0:$fee13,
			'fee14' => ($fee14 < 0)?0:$fee14,
			'fee15' => ($fee15 < 0)?0:$fee15,
			'fee16' => ($fee16 < 0)?0:$fee16,
			'fee17' => ($fee17 < 0)?0:$fee17,
			'fee18' => ($fee18 < 0)?0:$fee18,
			'fee19' => ($fee19 < 0)?0:$fee19,
			'fee20' => ($fee20 < 0)?0:$fee20,
			'fee21' => ($fee21 < 0)?0:$fee21,
			'fee22' => ($fee22 < 0)?0:$fee22,
			'fee23' => ($fee23 < 0)?0:$fee23,
			'fee24' => ($fee24 < 0)?0:$fee24,
			'fee25' => ($fee25 < 0)?0:$fee25,
		    'Collection_Mode' => 3,
			'Payment_Mode' => 'ONLINE',
			'Bank_Name' => 'CC Avenue',
            'User_Id'         => $adm_no,
