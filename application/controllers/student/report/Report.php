...
class Report extends MY_Controller {
	...
	public function daily_wise(){
		...
		$data['class_data'] = $this->alam->select_order_by('student_attendance_type','*','id');
				...
				foreach ($studentList as $key => $value) {
					...
					for ($i=1; $i <= $total_days; $i++) { 
						...
						if($checkSunday  == 7 || $checkHoliday == true)
						{
							...
							//change 24/9/24
							$checkSundey = $this->sumit->fetchSingleData('*', 'stu_attendance_entry', "admno='" . $value['ADM_NO'] . "' AND date(att_date)='$custom_date'");
							$attdate = $checkSundey['att_status'];
							if ($attdate == 'P' || $attdate == 'A') {
								$att_data_array[$value['ADM_NO']][$i]['status'] = $attdate;
							} else {
