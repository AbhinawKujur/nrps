...
class Attendancepercentagereport extends MY_Controller {
	...
	public function index()
	{
		...
		$data['user_id']=$user_id = login_details['user_id'];
		//$data['classList'] = $this->alam->selectA('class_section_wise_subject_allocation','distinct(Class_no),(select CLASS_NM from classes where Class_No=class_section_wise_subject_allocation.Class_No)classnm',"Main_Teacher_Code='$user_id'");
		$data['classList'] = $this->alam->selectA('login_details','class_no,section_no,(select class_nm from classes where class_no=login_details.Class_No)disp_class,(select section_name from Sections where Section_No=login_details.section_no)disp_sec',"user_id='$user_id'");
