...
class Teacher_master extends MY_Controller
{
	...
	public function clswise_subj_allco()
	{
		...
		$array = array('classes' => $classes, 'subs' => $subs, 'log_cls_no' => $log_cls_no, 'log_sec_no' => $log_sec_no);
...
?>
				...
				<tr style='padding:2px!important;' id='row_<?php echo $i; ?>'>
					...
					<td>
						<select id='optcode_<?php echo $i; ?>' name='opt_code[]' style="padding:2px; width:174px;" disabled>
							...
							<option value='0' <?php if ($conv == 0) {
													echo "selected";
												} ?>>Main</option>
							<option value='1' <?php if ($conv == 1) {
													echo "selected";
												} ?>>Optional</option>
							<option value='2' <?php if ($conv == 2) {
													echo "selected";
												} ?>>Sub-Optional</option>
						</select>
					</td>
	...
	public function allco_stu_list()
	{
		...
		$subj_code6 = $this->input->post('subj_code6');
		//echo '<pre>';
		//print_r($_POST);die;
		$stutbl    = $this->dbcon->stu_list_subj_allocation($classs, $sec, $subj_code6);
		...
		<div class="table-responsive">
			<table class="table">
				<tr style="background:#5785c3;">
					...
					<th style="color:#fff!important;">Subject5</th>
					<th style="color:#fff!important;">Subject6</th>
				</tr>
				<?php
				...
				?>
					<tr>
						<td>
							<input type="checkbox" name='adm_no[]' value="<?php echo $stu_data->ADM_no; ?>" onclick="chk_one()">
						</td>
						<td>
							<?php echo $stu_data->ADM_no; ?>
							<input type="hidden" name="class" value="<?php echo $classs; ?>">
							<input type="hidden" name="sec" value="<?php echo $sec; ?>">
							<input type="hidden" name="subj_code" value="<?php echo $subj_code; ?>">
							<input type="hidden" name="subj_code6" value="<?php echo $subj_code6; ?>">
						</td>
						<td><?php echo $stu_data->FIRST_NM; ?></td>
						<td><?php echo $stu_data->ROLL_NO; ?></td>
						<td><?php if ($stu_data->subnm5 != '') {
								echo $stu_data->subnm5;
							} else {
								echo "--";
							} ?></td>
						<td><?php if ($stu_data->subnm6 != '') {
								echo $stu_data->subnm6;
							} else {
								echo "--";
							} ?></td>
					</tr>
	...
	public function save_and_upd()
	{
		...
		for ($i = 0; $i < count($this->input->post('adm_no')); $i++) {
			...
			if ($cnt == 1) {
				...
				$this->dbcon->update("studentsubject", $data, "Adm_no='$adm_no' AND Class = '$class' AND SEC = '$sec' AND SUBCODE = $subj_code");

				$data = array(
					'SUBCODE' => $subj_code6
				);

				$this->dbcon->update("studentsubject", $data, "Adm_no='$adm_no' AND Class = '$class' AND SEC = '$sec' AND SUBCODE = $subj_code6");
				...
				$this->dbcon->insert('studentsubject', $data_ins1);
	...
	public function max_marks_allco()
	{
		...
		$array = array('classes' => $classes, 't1' => $t1, 't2' => $t2);
	...
	public function examination()
	{
		...
		if ($examcode == 1 || $examcode == 2 || $examcode == 3 || $examcode == 6 || $examcode == 7 || $examcode == 8 || $examcode == 9 || $examcode == 10 || $examcode == 11 || $examcode == 12 || $examcode == 13) {
	...
	public function save_upd_max_marks()
	{
		...
		if ($cnt == 0) {
			...
			$sqldt = $this->dbcon->select('marks', '*', "ExamC = '$examcode' AND SCode = '$subcode' AND Classes = '$classcode' AND Term = '$termm'");
			$cntt = count($sqldt);
			if ($cntt == 0) {
