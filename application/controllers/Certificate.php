...
class Certificate extends MY_Controller
{
	...
	public function details_show()
	{
		...
		if ($year == $session_year1) {
			...
			$back_data = $this->dbcon->select('session_master', '*', "Session_Nm='$year'");
			$student_data = $back_data[0]->student_table;

			$student_details = $this->dbcon->select($student_data, '*', "ADM_NO='$adm_no'");
			$tc_isu = $student_details[0]->TC_ISSUED;
	...
	public function student_details_27_07_2023()
	{
		$adm_no = $this->input->post('adm_no');
	...
	{
		$adm_no = $this->input->post('adm_no');
		$year = $this->session->userdata('session_year'); //year from modal that we choose 2021-2022 ,2022-2023 ,2023-2024
		$ses_year = $this->dbcon->select('session_master', '*', "Active_Status=1");
		$ses_allyear = $this->dbcon->select('session_master', '*');

		$session_year1 = $ses_year[0]->Session_Nm; //2023-2024
		...
		foreach ($ses_allyear as $allyear) {
			if ($year == $allyear->Session_Nm) {
				$tbl = $allyear->student_table;
			}
		}
		...
		if ($year == $session_year1) {
			...
			$student = $this->dbcon->get_student($tbl, $adm_no);
		}
	...
	public function tc_datafetch()
	{
		...
		$array = array(
			...
			'APAR_ID'		=> $this->input->post('aparid'),
		);
		...
		if ($year == $session_year1) {
			...
			$this->dbcon->update($student_data, $mont_update, "adm_no='$adm_no'");
		}
	...
	public function pdf_creation($id)
	{
		...
		$this->dompdf->render();
		$this->dompdf->stream($id . "-TC.pdf", array("Attachment" => 0));
	}
	public function cancel_reprint_tc()
	{
		$ses = $this->session->userdata('session_set');
	...
	// 15-11-22
	public function cancel_reprint_tc_pdf()
	{
		$ses = $this->session->userdata('session_set');
		$data = $this->dbcon->select('tc', '*');
		$School_setting = $this->dbcon->select('school_setting', '*');
		...
		$this->load->view('certificate/show_tcissue_pdf', $array);
		$html = $this->output->get_output();
		$this->load->library('pdf');
		$this->dompdf->loadHtml($html);
		$this->dompdf->setPaper('A4', 'portrait');
		$this->dompdf->render();
		$this->dompdf->stream("TC_Isued_List.pdf", array("Attachment" => 0));
	}
	public function show_cancel_reprint_tc($adm, $adm2, $adm3)
	{
		$adm = $adm . '/' . $adm2 . '/' . $adm3;
		$ses = $this->session->userdata('session_set');
		$tc_details = $this->dbcon->select('tc', '*,(select word_nm from classes_word_nm where tc.current_Class=classes_word_nm.CLASS_NM)current_Class_word,(select word_nm from classes_word_nm where tc.datacombo011=classes_word_nm.CLASS_NM)datacombo011_word,(select word_nm from classes_word_nm where tc.ADM_CLASS=classes_word_nm.CLASS_NM)ADM_CLASS_word', "adm_no='$adm'");
		$school_setting = $this->dbcon->select('school_setting', '*');
		$session_data = $this->dbcon->select('session_master', '*');
		...
		//	<script>
		//	alert('This Tc Is Already Cancelled');
		//window.close();</script>";
	...
	public function re_printpdf($adm_no)
	{
		// Sample string
		$adm_str_new = '';
		$adm_str = explode("_", $adm_no);
		foreach ($adm_str as $p) {
			$adm_str_new .= $p . "/";
		}
		$final_adm = rtrim($adm_str_new, "/");
		$tc_issue = $this->dbcon->select('tc', 'duplicate_issue', "adm_no='$final_adm'");
		//$adm_no=$adm_no.'/'.$adm2;
		$tc_count = $tc_issue[0]->duplicate_issue + 1;
		$array = array('duplicate_issue' => $tc_count);
		if ($this->dbcon->update('tc', $array, "tcnonum='$final_adm'")) {

			$tc_details = $this->dbcon->select('tc', '*', "adm_No='$final_adm'");
			$school_setting = $this->dbcon->select('school_setting', '*');
			$details_array = array(
				'tc_details' => $tc_details,
				'school_setting' => $school_setting
			);
			// echo"<pre>"; print_r($details_array)."kj";die;
			...
			$this->dompdf->setPaper('A4', 'portrait');
			$this->dompdf->render();
			$this->dompdf->stream($adm_no . "-TC-duplicate.pdf", array("Attachment" => 0));
		}
	}
	public function re_pre_printpdf($adm_no)
	{
		// Sample string
		$adm_str_new = '';
		$adm_str = explode("_", $adm_no);
		...
		if ($this->dbcon->update('tc', $array, "tcnonum='$final_adm'")) {
			...
			$this->load->view('certificate/cancel_pre_reprint_tc_pdf', $details_array);
	...
	{
		$adm_no  = $this->input->post('adm_no');
		$tc_fy = $this->dbcon->select('tc', 'session_year', "adm_no=(select ADM_NO from student where STUDENTID='$adm_no')");
		...
		if ($session_year == $session_year1) {
			$student_details = $this->dbcon->select('student', '*', "STUDENTID='$adm_no'");
			$mon[0] = $student_details[0]->APR_FEE;
			$mon[1] = $student_details[0]->MAY_FEE;
			$mon[2] = $student_details[0]->JUNE_FEE;
			$mon[3] = $student_details[0]->JULY_FEE;
			...
			if ($this->dbcon->update('student', $mont_update, "STUDENTID='$adm_no'") && $this->dbcon->update('tc', $tc_details_update, "adm_no=(select ADM_NO from student where STUDENTID='$adm_no')")) {
			...
			$student_details = $this->dbcon->select($data_baseName . '.student', '*', "STUDENTID='$adm_no'");
			$mon[0] = $student_details[0]->APR_FEE;
			$mon[1] = $student_details[0]->MAY_FEE;
			$mon[2] = $student_details[0]->JUNE_FEE;
			$mon[3] = $student_details[0]->JULY_FEE;
			...
			if ($this->dbcon->update($data_baseName . '.student', $mont_update, "STUDENTID='$adm_no'") && $this->dbcon->update('tc', $tc_details_update, "(select ADM_NO from student where STUDENTID='$adm_no')")) {
				$this->session->set_flashdata('msg', "Successfully Tc Cancelled");
				redirect('Certificate/cancel_reprint_tc');
			} else {
				$this->session->set_flashdata('msg', "Failed Tc Cancelled");
	...
	public function char_details_show()
	{
		...
		$ses_year = $this->dbcon->select('session_master', '*', "Active_Status=1");
		$session_year1 = $ses_year[0]->Session_Nm;
		// echo 'current ses = '.$session_year1 ;
		...
		// }
			...
			// $student = $this->dbcon->get_student($tbl, $adm_no);
			$student = $this->dbcon->select("$student_data", '*', "ADM_NO='$adm_no'");
			$cnt = count($student);
		}
	...
	public function save_char_details()
	{
		...
		$array = array(
			...
			'Adm_Date' => $this->input->post('adm_date'),
			'End_DATE' => date('Y-m-d', strtotime($this->input->post('issue_date'))),
		...
		$this->load->view('certificate/char_report_preprinted', $details_array);
	}
	public function char_details_pdf($adm)
	{
		$adm_str_new = '';
		$adm_str = explode("_", $adm);
		foreach ($adm_str as $p) {
			$adm_str_new .= $p . "/";
		}
		$adm_no = rtrim($adm_str_new, "/");
		// echo $adm_no;
		// die;
		$school_setting = $this->dbcon->select('school_setting', '*');
		$details_fetch = $this->dbcon->select('charcert', '*', "ADM_NO='$adm_no'");
		...
		$this->load->view('certificate/char_report_pdf', $details_array);
		$html = $this->output->get_output();
		$this->load->library('pdf');
		$this->dompdf->loadHtml($html);
		$this->dompdf->setPaper('A4', 'portrait');
		$this->dompdf->render();
		$this->dompdf->stream($adm_no . "-character.pdf", array("Attachment" => 0));
	}
	public function re_print_char($admo)
	{
		$adm_no = str_replace('_', '/', $admo);
		$details_fetch = $this->dbcon->select('charcert', '*', "ADM_NO='$adm_no'");
		$school_setting = $this->dbcon->select('school_setting', '*');
		$details_array = array(
			'details_fetch' => $details_fetch,
