...
class Bus_transport extends MY_Controller
{
	...
	public function show_student()
	{
		...
		$data['adm_no'] = $adm_no = $tmp[0] . '/' . $tmp[1] . '/' . $tmp[2];
		$adm_no_one = $tmp[0] . '_' . $tmp[1];
		...
		if (empty($data['trans_report'])) {
			...
			redirect('student_transport/Bus_transport/allocate_bus_stoppage/' . $adm_no_one, refresh);
		}
	...
	public function allocate_stopage()
	{
		...
		if ($MON == "none") {
			...
			redirect('student_transport/Bus_transport/show_student', refresh);
		}
		if ($NEW_STOPNO == "none") {
		...
		if ($this->db->trans_status() === FALSE) {
			...
			redirect('Student\Bus_transport', refresh);
		} else {
