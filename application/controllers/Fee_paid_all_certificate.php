...
class Fee_paid_all_certificate extends MY_Controller
{
	...
	public function fee_details_pdf($admo)
	{
		...
		$details_array = array(
			'ADM_NO'		 => $adm_no,
			'S_NAME'		 => $student[0]->FIRST_NM,
			'F_NAME'		 => $student[0]->FATHER_NM,
			'M_Name' 		 => $student[0]->MOTHER_NM,
			'Birth_Date' 	 => $student[0]->BIRTH_DT,
			'class_name' 	 => $student[0]->DISP_CLASS,
			'sec' 	 		 => $student[0]->DISP_SEC,
			'fee_paid_from'  => "APR",
			'session_year'  => $session_year,
			'upto'           => $final_month,
			'total_paid' 	 => $daycoll[0]->TOTAL,
			'school_setting' => $school_setting,
			'daycoll' => $daycoll,
			'feehead' => $feehead,
			'fee_details' => $fee_details
		...
		$this->load->view('certificate/all_fee_report_pdf', $details_array);
		$html = $this->output->get_output();
		$this->load->library('pdf');
		$this->dompdf->loadHtml($html);
		$this->dompdf->setPaper('A4', 'portrait');
		$this->dompdf->render();
		$this->dompdf->stream($adm_no . "-AllFeePaid.pdf", array("Attachment" => 0));
	}
}
