...
class Defaulter_headwise_list extends MY_Controller
{
	...
	public function defaulter_classwise()
	{
		...
		$result['text'] = $text;
		$student = $this->dbcon->select('student', 'ADM_NO,FIRST_NM,ROLL_NO,APR_FEE,MAY_FEE,JUNE_FEE,JULY_FEE,AUG_FEE,SEP_FEE,OCT_FEE,NOV_FEE,DEC_FEE,JAN_FEE,FEB_FEE,MAR_FEE,DISP_CLASS,DISP_SEC', "Student_Status='ACTIVE' AND CLASS='$classs' AND SEC='$sec' AND FREESHIP != 1");
	...
	public function defaulter_allclasswise()
	{
		...
		$result = array('data' => array());
		$feehead = $this->dbcon->select('feehead', 'SHNAME');
		$result['view'] = $viewupto;
		$result['clss'] = $classs;
		$result['sec'] = $sec;
		$result['feehead'] = $feehead;
		$result['text'] = $text;
		$student = $this->db->query("Select ADM_NO,FIRST_NM,ROLL_NO,APR_FEE,MAY_FEE,JUNE_FEE,JULY_FEE,AUG_FEE,SEP_FEE,OCT_FEE,NOV_FEE,DEC_FEE,JAN_FEE,FEB_FEE,MAR_FEE,DISP_CLASS,DISP_SEC from student where student_status='ACTIVE' and disp_Sec not in ('K','DT','RT','TC','Z') and freeship != 1  order by class,sec,first_Nm asc")->result();
		...
		for ($i = 0; $i < $student_cnt; $i++) {
			...
			$total_amt_fee1 = $fee1_unpaid_month;
			$total_amt_fee2 = $fee2_unpaid_month;
			$total_amt_fee3 = $fee3_unpaid_month;
			$total_amt_fee4 = $fee4_unpaid_month;
			$total_amt_fee5 = $fee5_unpaid_month;
			$total_amt_fee6 = $fee6_unpaid_month;
			$total_amt_fee7 = $fee7_unpaid_month;
			$total_amt_fee8 = $fee8_unpaid_month;
			$total_amt_fee9 = $fee9_unpaid_month;
			$total_amt_fee10 = $fee10_unpaid_month;
			$total_amt_fee11 = $fee11_unpaid_month;
			$total_amt_fee12 = $fee12_unpaid_month;
			$total_amt_fee13 = $fee13_unpaid_month;
			$total_amt_fee14 = $fee14_unpaid_month;
			$total_amt_fee15 = $fee15_unpaid_month;
			$total_amt_fee16 = $fee16_unpaid_month;
			$total_amt_fee17 = $fee17_unpaid_month;
			$total_amt_fee18 = $fee18_unpaid_month;
			$total_amt_fee19 = $fee19_unpaid_month;
			$total_amt_fee20 = $fee20_unpaid_month;
			$total_amt_fee21 = $fee21_unpaid_month;
			$total_amt_fee22 = $fee22_unpaid_month;
			$total_amt_fee23 = $fee23_unpaid_month;
			$total_amt_fee24 = $fee24_unpaid_month;
			$total_amt_fee25 = $fee25_unpaid_month;
	...
	public function defaulter_allclasswise_old()
	{
		ini_set('memory_limit', '-1');
		ini_set('max_execution_time', '0');
		$viewupto = $this->input->post('viewupto');
	...
	public function defaulter_classHeadWisePDF($view = '', $clss = '', $secc = '')
	{
		...
		if (!empty($view && $clss && $secc)) {
			$classs = $clss;
			$sec = $secc;
			$student = $this->dbcon->select('student', 'ADM_NO,FIRST_NM,ROLL_NO,APR_FEE,MAY_FEE,JUNE_FEE,JULY_FEE,AUG_FEE,SEP_FEE,OCT_FEE,NOV_FEE,DEC_FEE,JAN_FEE,FEB_FEE,MAR_FEE,DISP_CLASS,DISP_SEC', "Student_Status='ACTIVE' AND freeship != 1 AND CLASS='$classs' AND SEC='$sec'");
		} else {
			$student = $this->db->query("Select ADM_NO,FIRST_NM,ROLL_NO,APR_FEE,MAY_FEE,JUNE_FEE,JULY_FEE,AUG_FEE,SEP_FEE,OCT_FEE,NOV_FEE,DEC_FEE,JAN_FEE,FEB_FEE,MAR_FEE,DISP_CLASS,DISP_SEC from student where student_status='ACTIVE' AND freeship != 1 and disp_Sec not in ('K','DT','RT','TC','Z')  order by class,sec,first_Nm asc")->result();
		}
		$result = array('data' => array());
		$feehead = $this->dbcon->select('feehead', 'SHNAME');
		$result['feehead'] = $feehead;
		...
		//$this->dompdf->render();
		//$this->dompdf->stream("Head_Wise_Defaulter.pdf", array("Attachment" => 0));
	}
	public function defaulter_classHeadWisePDF_OLD($view = '', $clss = '', $secc = '')
	{
		ini_set('memory_limit', '-1');
		ini_set('max_execution_time', '0');
		// echo $view;die;
