...
class MarksReportstudent extends MY_controller{
	...
	public function index(){	
        ...
        $this->render_template('onlineexam/reports/marks_report_student',$array);		
	}
...
<?php 
	...
	public function generateMarksReport(){
		...
		$exam_schedule_id= $exam_shedule[0]['id'];
		 ...
		 (SELECT COUNT(id) FROM online_exam_question WHERE schedule_id=ose.exam_schedule_id and exam_ptern_id=2) AS totalmcq, 
(SELECT COUNT(id) FROM online_exam_answers WHERE exam_schedule_id=ose.exam_schedule_id AND admno=st.ADM_NO  AND ans_status='1' and  exam_pattern=2) AS totalattampt,
(SELECT sum(stu_marks) FROM online_exam_answers WHERE exam_schedule_id=ose.exam_schedule_id AND admno=st.ADM_NO  AND ans_status='1' and  exam_pattern=2) AS student_marks,(SELECT count(id) FROM online_exam_answers WHERE exam_schedule_id=ose.exam_schedule_id AND admno=st.ADM_NO  AND ans_status='1'  and stu_marks>0 and  exam_pattern=2) AS totalcorrect,(SELECT count(id) FROM online_exam_answers WHERE exam_schedule_id=ose.exam_schedule_id AND admno=st.ADM_NO  AND ans_status='1'  and stu_marks=0 and  exam_pattern=2) AS totalincorrect FROM student AS st INNER JOIN online_student_exam_list AS ose ON st.ADM_NO=ose.admno WHERE ose.class_id='$classes' AND ose.sec_id='$section_id' AND exam_schedule_id='$exam_schedule_id' order by $sr")->result();		 
   ...
   $schoolset	=$this->pawan->selectA('school_setting','*');
	?>
		...
		<table class='table table2excel' width="100%" cellspacing="0" style='font-family: verdana;'>
			<tr>					
					<td style="background:#5785c3; color:#fff!important;border-left: 1px solid;border-bottom:1px solid"></td><td style="background:#5785c3; color:#fff!important;border-right: 1px solid;border-bottom:1px solid" colspan="7"><center style="font-size:17px"><strong><?=$schoolset[0]['School_Name'];?><br /><?=$schoolset[0]['School_Address'];?></strong><center></td>
					</tr>
					<tr>					
				...
				<tr>					
					...
					<!--<td style='background:#5785c3; color:#fff!important;border: 1px solid;width:100px;'><strong>Total Question</strong></td>--><td style='background:#5785c3; color:#fff!important;border: 1px solid;width:100px;'><strong>Total MCQ</strong></td>
				...
				<!--	<td style='background:#5785c3; color:#fff!important;border: 1px solid;'><strong>Total Incorrect</strong></td>-->
					<td style='background:#5785c3; color:#fff!important;border: 1px solid;width:100px;'><strong>Marks Obtained</strong></td>
				...
				<td style="border: 1px solid ;text-align: center;"><?=$val->totalmcq;?></td>
