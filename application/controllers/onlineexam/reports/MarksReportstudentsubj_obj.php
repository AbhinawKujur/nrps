...
class MarksReportstudentsubj_obj extends MY_controller{
	...
	public function index(){	
        ...
        $this->render_template('onlineexam/reports/marks_report_student_subj_obj',$array);		
	}
...
<?php 
	...
	public function generateMarksReport(){
		...
		//echo $this->db->last_query();die;
		 ...
		 $marks_max	=	$this->pawan->selectA('online_exam_question','sum(marks) as mark',"schedule_id='$exam_schedule_id'");
		$marks_max_sub	=	$this->pawan->selectA('online_exam_question','sum(marks) as mark',"exam_ptern_id='1' and schedule_id='$exam_schedule_id'");
		//echo $this->db->last_query(); die;
		$marks_max_obj	=	$this->pawan->selectA('online_exam_question','sum(marks) as mark',"exam_ptern_id='2' and schedule_id='$exam_schedule_id'");
...
(SELECT COUNT(id) FROM online_exam_answers WHERE exam_schedule_id=ose.exam_schedule_id AND admno=st.ADM_NO and  exam_pattern in ('1','2')) AS totalattampt,

(SELECT sum(stu_marks) FROM online_exam_answers WHERE exam_schedule_id=ose.exam_schedule_id AND admno=st.ADM_NO and  exam_pattern=2 and ans_status='1') AS student_marks_obj,
(SELECT sum(stu_marks) FROM online_exam_answers WHERE exam_schedule_id=ose.exam_schedule_id AND admno=st.ADM_NO and  exam_pattern=1 and teacher_final_copy_correction='2') AS student_marks_subj
FROM student AS st INNER JOIN online_student_exam_list AS ose ON st.ADM_NO=ose.admno WHERE ose.class_id='$classes' AND ose.sec_id='$section_id' AND exam_schedule_id='$exam_schedule_id'  order by $sr")->result();		 
		...
		<table class='table table2excel' width="100%" cellspacing="0" style='font-family: verdana;'>
			<tr>					
				...
				<tr style="font-size: 12px;">					
					...
					<td style='background:#5785c3; color:#fff!important;border: 1px solid;width:100px;text-align:center'><strong>Total Question</strong></td>
					<td style='background:#5785c3; color:#fff!important;border: 1px solid;width:100px;text-align:center'><strong>MCQ Marks Obtained<span style='color:#9c0707d6'><br>(Max-Marks:<?=$marks_max_obj[0]['mark']?>)</span></strong></td>
					<td style='background:#5785c3; color:#fff!important;border: 1px solid;width:100px;text-align:center'><strong>Subjective Marks Obtained<span style='color:#9c0707d6'><br>(Max-Marks:<?=$marks_max_sub[0]['mark']?>)</span></strong></td>
					...
					<td style='background:#5785c3; color:#fff!important;border: 1px solid;width:130px;text-align:center'><strong>Grand Total <span style='color:#9c0707d6'>(Max-Marks:<?=$marks_max[0]['mark']?>)</span></strong></td>
				...
				<td style="border: 1px solid ;text-align: center;"><?=$val->student_marks_obj;?></td>
				<td style="border: 1px solid ;text-align: center;"><?=$val->student_marks_subj;?></td>
				...
				<td style="border: 1px solid ;text-align: center;"><?=$val->student_marks_obj+$val->student_marks_subj;?></td>
				</tr>
	<?php	} ?>
</table>
<script>
	...
	$(document).ready(function(e) {				
				$("#myButton").click(function(e){
					$(".table2excel").table2excel({
						...
						filename: "marksSubjective_and_objective",
						fileext: ".xls",
						exclude_img: true,
						exclude_links: true,
						exclude_inputs: true
